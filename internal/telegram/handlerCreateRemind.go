...
func (t *Bot) handleCreateRemind(message *tgbotapi.Message) error {
	t.bot.Send(tgbotapi.NewMessage(t.curChatID, "О чём вам напомнить"))
	...
	for update := range t.updatesChan {
		if update.CallbackQuery != nil {
			...
			case "<":
				...
				t.bot.Send(tgbotapi.NewEditMessageReplyMarkup(t.curChatID, curInlineMarkupID, keyboard))
			case ">":
				var keyboard tgbotapi.InlineKeyboardMarkup
			...
			if len(data) == 10 {
				...
				logrus.Info("Date: ", remind.ActivationTime)
				if chosenDateMsgID == 0 {
					msg := tgbotapi.NewMessage(t.curChatID, fmt.Sprintf("Вы выбрали: %s\n", remind.ActivationTime.Format("2006.01.02")))
					...
					_, err := t.bot.Send(tgbotapi.NewEditMessageText(t.curChatID, chosenDateMsgID, fmt.Sprintf("Вы выбрали: %s\n", remind.ActivationTime.Format("2006.01.02"))))
					if err != nil {
						return err
					}
				}
		...
		if update.Message != nil {
			if update.Message.Text == "Отмена" {
				msg := tgbotapi.NewMessage(t.curChatID, "Создание напоминания отменено")
			...
			case 0:
				...
				remind.ChatID = t.curChatID
				lastID, err := t.db.GetLastRemindIDByChatID(t.curChatID)
				if err != nil {
					return err
				}
				remind.IDForChat = lastID + 1
				updateKeyboard := tgbotapi.NewMessage(t.curChatID, "...")
			...
			case 1:
				msg1 := tgbotapi.NewMessage(t.curChatID, "Выберите время:")
			...
			case 2:
				...
				// logrus.Println("user.TimeZoneOffset: ", user.TimeZoneOffset)
				msg1 := tgbotapi.NewMessage(t.curChatID, fmt.Sprintf(`Ваше напоминание:

				%s

				%s в %s`, remind.Text, remind.ActivationTime.Format("2006.01.02"), remind.ActivationTime.Format("15:04")))
				t.bot.Send(msg1)
				t.bot.Send(tgbotapi.NewMessage(t.curChatID, "Если всё верно - нажмите Далее"))
			case 3:
				remindRes, err := t.db.CreateRemind(*remind)
				if err != nil {
					return err
				}
				// logrus.Info("Until creation:", time.Until(remind.ActivationTime))
				if time.Until(remind.ActivationTime) < time.Hour {
					go t.serveRemind(remindRes)
				}
				msg := tgbotapi.NewMessage(t.curChatID, "Напоминание успешно создано!")
				msg.ReplyMarkup = keyboard.GetMainKeyboard()
				_, err = t.bot.Send(msg)
				return err
			}
